...
#define _PRINT_DEBUG_MIG_READY_  // uncomment for DEBUG print.

#ifdef _PRINT_DEBUG_MIG_READY_
#define DEBUG_PRINT(x) printf x
#else
#define DEBUG_PRINT(x) do {} while (0)
#endif

#define DEBUG_PRINT_VAL(x) DEBUG_PRINT(("[MIGRATION_READY] (%s) %s:%lx\n", __func__, #x, x))

static int stop_value;
static unsigned long* vmem_for_cpu;
struct regs_in_vmcs guest_riv;
struct regs_in_vmcs server_guest_riv;
...
load_guest_state_to_vmcs ()
{
	...
	mig_flag_for_mac = 1;
	printf("Migratino done!, mig_flag_for_mac is 1\n");
}
...
migration_ready_vmmcall()
{
	...
	while (1) {
		//usleep(5000000);
		//printf("migration_ready_vmmcall: 5 second passed\n");
		if(stop_value == 0)
			break;
		schedule();
	}
...
migration_ready_sub (unsigned long (*array)[2], int len)
{
	...
	printf("migration_ready_sub called\n");
