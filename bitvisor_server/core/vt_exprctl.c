...
exprctl_stop_measure(void)
{
	...
	static void
exprctl_print_vmruntime(param)
{
	printf ("Start of vmrun time\n");
...
exprctl_print_stat(void)
{
	if (num_request_sent != 0) {
		...
		printf ("(%s) num_request_sent: %lld\n", __func__,
				num_request_sent);
		printf ("(%s) avg recved by bg thread during requesting: %d\n", __func__, 
				sum_back_receive / num_request_sent);
		printf ("(%s) \n", __func__);
		printf ("(%s) request reply latency: %lld\n", __func__, sum_request_recv_t);
		printf ("(%s) request reply latency: %lld\n", __func__,
...
exprctl_sub (unsigned long (*array)[3], int len)
{
	...
	switch (cmd) {
		...
		case EXPR_PRINT_APPEXIT:
			...
			break;
		case EXPR_PRINT_STAT:
			printf ("Print statistics in %s\n", __func__);
			exprctl_print_stat();
			break;
		default:
			break;
