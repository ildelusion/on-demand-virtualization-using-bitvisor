...
#define SOURCE_CPU_STATE_MEMORY_ADDR 0x49005000

#define DEBUG_MIG

#define _PRINT_DEBUG_MIG_	// uncomment for DEBUG print.

#ifdef _PRINT_DEBUG_MIG_
#define DEBUG_PRINT(x) printf x
#else
#define DEBUG_PRINT(x) do {} while (0)
#endif

#define DEBUG_PRINT_VAL(x) DEBUG_PRINT(("[MIGRATION_VMMCALL] (%s) %s:%lx\n", __func__, #x, x))
...
map_guest_cpu_state_area(void)
{
	...
	mem = (unsigned long*)mapmem(MAPMEM_HPHYS | MAPMEM_WRITE, SOURCE_CPU_STATE_MEMORY_ADDR, 0x1000);
	ASSERT(mem);
	return mem;
}
...
migration_vmmcall ()
{
	...
	// Deactivate virtio backend driver.
	printf ("migration vmmcall called.\n");
	...
	printf ("Let's unregister virtio_handler.\n");
	core_io_unregister_virtio_handler();

	// CPU, memory, network states transfer
	
	for(i = 0; i< NUM_OF_MEM_AREA; i++) {
		num_of_send[i] = usable_mem_list[i].end_addr - usable_mem_list[i].start_addr + 1;
		num_of_send[i] /= SIZE_OF_SENDBUF;
		total_num_of_send += num_of_send[i];
	}

	// map 0x49005000
	mem = map_guest_cpu_state_area();
	printf("[MIG] mapped memory: %lx\n", mem);
	save_guest_cpu_state(mem);

	// Send message to echoctl.
	send_msg_to_echoctl (mem, total_num_of_send);
	get_acpi_time(&time2);
	printf("migration ready time: %llu\n", time2 - time1);

	// It should be done in target machine.
	/*
	printf ("Let's register virtio_handler.\n");
	core_io_register_virtio_handler();*/
	//printf ("migration vmmcall ended. Get into infinite looping.\n");
	while(1){
		schedule();
		/*if(mig_done == 1) {
			printf("mig done!\n");
	...
	core_io_register_virtio_handler();
	printf("migration_vmmcall: core_io_register_virtio_handler\n");
	vmcs_dump_vcpu();
}
